...
class Volume_sample_program
{
    ...
    const float grad_max        = 20.0f;        // maximum gradient scale
    const float grad_min        = 0.2f;        // minimum gradient scale
...
public:
              ...
              Sample_output&     sample_output)
    {
        ...
        const float user_input = 0.8;
        const float grad_scale = max(user_input, grad_min) * grad_max; 
        ...
        else
        {
            ...
            const float grad_mag = length(vol_grad);
            sample_output.color  = clamp(sample_color * min((grad_mag * grad_scale),1.5f),0.0f,1.0f);
    ...
    NV_IDX_DEVICE_INLINE_MEMBER
    float3 get_gradient(const Sparse_volume_sampler<float, Volume_filter_mode::TRILINEAR, Volume_classification_mode::POST_INTERPOLATION>& sampler, const float3 sample_position, const float dh)
    {
